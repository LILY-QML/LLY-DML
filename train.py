...
"""
    ...
    def execute_training(self, all_optimizers=True, specific_optimizers=None):
        ...
        # Auswahl der zu trainierenden Optimizer
        if all_optimizers:
            optimizers_to_train = self.get_all_available_optimizers()
        elif specific_optimizers:
            optimizers_to_train = specific_optimizers
        else:
            optimizers_to_train = []

        # Iteriere durch jeden Optimizer
        for optimizer_name in optimizers_to_train:
            print(f"\n===== Optimizer: {optimizer_name} =====")
            self.reader.logger.info(f"Starting training for optimizer: {optimizer_name}")
            
            # Initialisiere den Optimizer
            try:
                optimizer = OptimizerManager.load_optimizer(
                    optimizer_name=optimizer_name,
                    data=self.data,
                    training_matrix=self.training_matrix,
                    target_state='000111',  # Passe den Zielzustand nach Bedarf an
                    logger=self.reader.logger,
                    **self.get_optimizer_kwargs(optimizer_name)
                )
                print(f"Optimizer {optimizer_name} loaded successfully.")
                self.reader.logger.info(f"Optimizer {optimizer_name} loaded successfully.")
            except ImportError as e:
                print(f"Fehler beim Laden des Optimizers {optimizer_name}: {e}")
                self.reader.logger.error(f"Fehler beim Laden des Optimizers {optimizer_name}: {e}")
                continue  # Springe zum nächsten Optimizer

            # Iteriere durch jede Aktivierungsmatrix
            for idx, activation_matrix in enumerate(self.reader.activation_matrices, start=1):
                print(f"\n--- Optimizing with Activation Matrix {idx} ---")
                self.reader.logger.info(f"Optimizer {optimizer_name}: Optimizing with Activation Matrix {idx}")

                # Setze die Aktivierungsmatrix im Circuit
                try:
                    self.circuit.set_activation_matrix(activation_matrix)
                    self.reader.logger.debug(f"Set activation matrix {idx} for optimizer {optimizer_name}.")
                    print(f"Activation Matrix {idx} set in the circuit.")
                except Exception as e:
                    print(f"Fehler beim Setzen der Aktivierungsmatrix {idx}: {e}")
                    self.reader.logger.error(f"Fehler beim Setzen der Aktivierungsmatrix {idx}: {e}")
                    continue  # Springe zur nächsten Aktivierungsmatrix

                # Führe den Circuit aus und erhalte den Output
                try:
                    output = self.circuit.execute(training_matrix=self.training_matrix)
                    self.reader.logger.debug(f"Circuit executed with Activation Matrix {idx} for optimizer {optimizer_name}.")
                    print(f"Circuit executed. Output obtained.")
                except Exception as e:
                    print(f"Fehler beim Ausführen des Circuits mit Aktivierungsmatrix {idx}: {e}")
                    self.reader.logger.error(f"Fehler beim Ausführen des Circuits mit Aktivierungsmatrix {idx}: {e}")
                    continue  # Springe zur nächsten Aktivierungsmatrix

                # Optimieren der Trainingsmatrix basierend auf dem Output
                try:
                    optimized_training_matrix = optimizer.optimize(training_matrix=self.training_matrix, output=output)
                    self.reader.logger.debug(f"Optimizer {optimizer_name} optimized the training matrix with Activation Matrix {idx}.")
                    print(f"Training matrix optimized by {optimizer_name}.")
                except Exception as e:
                    print(f"Fehler bei der Optimierung der Trainingsmatrix mit {optimizer_name}: {e}")
                    self.reader.logger.error(f"Fehler bei der Optimierung der Trainingsmatrix mit {optimizer_name}: {e}")
                    continue  # Springe zur nächsten Aktivierungsmatrix

                # Aktualisiere die Trainingsmatrix im Circuit
                try:
                    self.circuit.set_training_matrix(optimized_training_matrix)
                    self.training_matrix = optimized_training_matrix  # Aktualisiere die lokale Variable
                    self.reader.logger.debug(f"Training matrix updated in the circuit by {optimizer_name} with Activation Matrix {idx}.")
                    print(f"Training matrix updated in the circuit.")
                except Exception as e:
                    print(f"Fehler beim Aktualisieren der Trainingsmatrix im Circuit: {e}")
                    self.reader.logger.error(f"Fehler beim Aktualisieren der Trainingsmatrix im Circuit: {e}")
                    continue  # Springe zur nächsten Aktivierungsmatrix

            # Nach dem Training mit allen Aktivierungsmatrizen, speichere die optimierte Trainingsmatrix
            self.save_optimized_training_matrix(optimizer_name)

        # Nach dem Training aller Optimizer, führe Messungen mit jeder Aktivierungsmatrix durch
        self.perform_measurements()
        input("Press Enter to continue.")

    def perform_measurements(self):
        """Führt Messungen mit jeder Aktivierungsmatrix durch und speichert die Ergebnisse."""
        self.clear_console()
        print("\nPERFORMING MEASUREMENTS")
        self.reader.logger.info("Starting measurements with each activation matrix.")

        measurement_results = {}

        for idx, activation_matrix in enumerate(self.reader.activation_matrices, start=1):
            print(f"\n--- Measurement with Activation Matrix {idx} ---")
            self.reader.logger.info(f"Performing measurement with Activation Matrix {idx}.")

            # Setze die Aktivierungsmatrix im Circuit
            try:
                self.circuit.set_activation_matrix(activation_matrix)
                self.reader.logger.debug(f"Set activation matrix {idx} for measurement.")
                print(f"Activation Matrix {idx} set in the circuit.")
            except Exception as e:
                print(f"Fehler beim Setzen der Aktivierungsmatrix {idx} für Messung: {e}")
                self.reader.logger.error(f"Fehler beim Setzen der Aktivierungsmatrix {idx} für Messung: {e}")
                continue  # Springe zur nächsten Aktivierungsmatrix

            # Führe den Circuit aus und erhalte die Messungen
            try:
                measurements = self.circuit.execute(training_matrix=self.training_matrix, perform_measurement=True)
                self.reader.logger.debug(f"Measurements obtained with Activation Matrix {idx}.")
                print(f"Measurements obtained.")
                measurement_results[f"Activation_Matrix_{idx}"] = measurements.tolist()
            except Exception as e:
                print(f"Fehler beim Ausführen des Circuits für Messung mit Aktivierungsmatrix {idx}: {e}")
                self.reader.logger.error(f"Fehler beim Ausführen des Circuits für Messung mit Aktivierungsmatrix {idx}: {e}")
                continue  # Springe zur nächsten Aktivierungsmatrix

        # Speichere die Messungsergebnisse in train.json
        try:
            with open(self.train_file, 'r') as f:
            ...
            self.reader.logger.error(f"Error decoding train.json: {e}")
            print(f"Error decoding train.json: {e}")
            sys.exit(1)
        except Exception as e:
            self.reader.logger.error(f"Error reading train.json: {e}")
            print(f"Error reading train.json: {e}")
            sys.exit(1)

        # Füge die Messungsergebnisse hinzu
        train_data['simulation_results'] = measurement_results

        # Schreibe die aktualisierten Daten zurück in train.json
        try:
            with open(self.train_file, 'w') as f:
                json.dump(train_data, f, indent=4)
            self.reader.logger.info("Measurement results saved to train.json.")
            print("Measurement results saved to train.json.")
        except Exception as e:
            self.reader.logger.error(f"Fehler beim Schreiben der Messungsergebnisse in train.json: {e}")
            print(f"Fehler beim Schreiben der Messungsergebnisse in train.json: {e}")
            sys.exit(1)

    def save_optimized_training_matrix(self, optimizer_name):
        """Speichert die optimierte Trainingsmatrix in train.json unter dem entsprechenden Optimizer."""
        try:
            with open(self.train_file, 'r') as f:
                train_data = json.load(f)
        except FileNotFoundError:
        ...
        # Füge den Optimizer-Schritt hinzu
        optimizer_step = {
            "optimizer": optimizer_name,
            "optimized_training_matrix": self.training_matrix.tolist(),
            "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            # Weitere relevante Informationen können hier hinzugefügt werden
        }
        train_data.setdefault('optimizer_steps', []).append(optimizer_step)

        # Schreibe die aktualisierten Daten zurück in train.json
        try:
            with open(self.train_file, 'w') as f:
                json.dump(train_data, f, indent=4)
            self.reader.logger.info(f"Optimierte Trainingsmatrix für {optimizer_name} in train.json gespeichert.")
            print(f"Optimierte Trainingsmatrix für {optimizer_name} in train.json gespeichert.")
        except Exception as e:
            self.reader.logger.error(f"Fehler beim Schreiben in train.json: {e}")
            print(f"Fehler beim Schreiben in train.json: {e}")
            sys.exit(1)
