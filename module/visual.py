...
class Visual:
    ...
    def add_initiated_data(self, story):
        ...
        story.append(Paragraph("<a name='section1'/>1. Initiated Data", self.styles['Heading2']))
        story.append(Spacer(1, 20))

        # Initial Quantum Circuit
        circuit_image_path = os.path.join("var", "circuit_initial_1.png")
        story.append(Paragraph("Initial Quantum Circuit:", self.styles['Heading3']))
    ...
    def add_optimization_methods(self, story):
        ...
        story.append(Paragraph("<a name='section2'/>2. List of Optimization Methods", self.styles['Heading2']))
        story.append(Spacer(1, 20))
    ...
    def add_comparison_section(self, story):
        ...
        story.append(Paragraph("<a name='section3'/>3. Comparison Between Methods", self.styles['Heading2']))
        story.append(Spacer(1, 20))

        # Create a DataFrame for comparison
        comparison_df = pd.DataFrame(self.results)
        comparison_df['Improvement'] = comparison_df['Final Probability'] - comparison_df['Initial Probability']
        
        # Plot comparison as heatmap
        plt.figure(figsize=(10, 8))
        heatmap_data = comparison_df.pivot_table(values="Improvement", index="Optimizer", columns="Target State")
        ...
        table_data = [[str(i) for i in row] for row in comparison_df.round(4).values.tolist()]  # Convert all elements to strings
    ...
    def add_final_results_section(self, story):
        ...
        story.append(Paragraph("<a name='section4'/>4. Final Results", self.styles['Heading2']))
        story.append(Spacer(1, 20))
...
class TitlePage:
    def __init__(self, title, subtitle, copyright_info, description, date, additional_info):
        self.title = title
    ...
    def build(self, story, styles):
        ...
        big_heading_paragraph = Paragraph("QUANTUM LLY-DML TRAINING REPORT", big_heading_style)
        ...
        story.extend([
            title_paragraph,
            subtitle_paragraph,
            copyright_paragraph, Spacer(1, 40),  # Adjusted space between copyright and description
            description_paragraph, Spacer(1, 20),
            big_heading_paragraph,
            additional_info_paragraph, Spacer(1, 40),
            date_paragraph, Spacer(1, 40),
...
class OptimizationMethod:
    ...
    def build(self, story, styles):
        ...
        method_use_case = Paragraph(f"<b>Use Case:</b> {self.use_case}", styles['Normal'])

        # Adding content to the story
        story.extend([
            method_title, Spacer(1, 10),
            method_description, Spacer(1, 5),
            method_use_case, Spacer(1, 20),
        ])
...
class ComparisonSection:
    ...
    def build(self, story, styles):
        ...
        # Adding content to the story
        story.extend([
            comparison_title, Spacer(1, 20),
            comparison_content, Spacer(1, 40),
        ])
...
class FinalResultsSection:
    ...
    def build(self, story, styles):
        ...
        # Adding content to the story
        story.extend([
            final_results_title, Spacer(1, 20),
            final_results_content, Spacer(1, 40),
        ])
